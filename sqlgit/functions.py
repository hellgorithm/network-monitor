...
def OpenConnection(connectWindow):
	...
	if stat:
		...
		configPath = os.path.expanduser("~") + "/sqlgit/sqlgit-config.xml"
...
def configureDB(connectWindow):
	try:
		...
		error_message = "SQLGit scripts not found. Click OK to install."
		...
		if reply == QtWidgets.QMessageBox.Ok:
			...
			if globalvars.engine == "Microsoft SQL Server":
				...
				cursor = conn.cursor()
				cursor.execute("CREATE DATABASE SQLGit;")
				conn.close()
		else:
			return False
...
def saveConfigurations(path, connWin = None):
	...
	if os.path.exists(path):
		...
		directory = os.path.expanduser("~") + "/sqlgit/"
